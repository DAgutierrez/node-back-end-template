...
module.exports = function(db) {
    ...
    var oauth2Model = {
        getClient : function(clientId, clientSecret, cb) {
            ...
            db.query('SELECT ClientId, ClientSecret, RedirectUri FROM OauthClients WHERE ClientId = :ClientId',
        ...
        where (Users.Email = :Email) and Users.Password = :Password and Systems.ClientId = :ClientId',
                { replacements: { Email: username, Password: hashPassword(password),ClientId : currentClientId } }
            )
                .spread( function(res) {
        ...
        VALUES (:AccessToken, :ClientId, :UserId, :Expires, NOW(), NOW())',
                { replacements: { AccessToken: accessToken, ClientId: clientId,UserId : userId[0].id,Expires : expires  } }
                )
                .spread(function(res) {
        ...
        getAccessToken : function(bearerToken, cb) {
            ...
            db.query(
                ...
                { replacements: { BearerToken: bearerToken, } }
            )
            .spread( function(res) {
                if(!res.length) return cb(null);
                var token = res[0];
        ...
        VALUES (:RefreshToken, :ClientId, :UserId, :Expires, NOW(), NOW())',
                { replacements: { RefreshToken: token, ClientId: clientId, UserId : userId[0].id, Expires: expires } }
             )
                .spread(function(res) {
                    cb(null);
                }, function(err) {
        ...
        getRefreshToken : function(bearerToken, cb) {
            ...
            db.query(
                'SELECT RefreshToken, ClientId, Expires, UserId \
                FROM OauthRefreshTokens \
                WHERE RefreshToken = :RefreshToken',
                { replacements: { RefreshToken: bearerToken } }
            )
                .spread( function(res) {
                    if(!res.length) return cb(null);
                    var token = res[0];
